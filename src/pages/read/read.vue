  ...
  <div ref="bookRef" :class="[{'black':isShwNight},bgColor]">
    <div>
      <van-overlay
          ...
          @click="show = false;settingShow=false"
      />
      ...
      <div class="settingRow" v-show="settingShow">
        <div>
          ...
          <span class="settSpan" :class="{disabled:fontSize >= maxFontSize}" @click="changeFontSize(0)">A+</span>
          <span class="settSpan" :class="{disabled:fontSize <= minFontSize}" @click="changeFontSize(1)">A-</span>
        </div>
        <div>
          <span class="settName">背景</span>
          <span
              class="settSpan" v-for="(item,index) in bgColorList" :key="index" :class="[item,{active:bgColor===item}]"
    ...
    <!--头部title-->
    <van-sticky>
      <p class="col-12 topTitle" :class="[{'black':isShwNight,'white':isShwNight},bgColor]" @click="show = !show">{{
        chapter.title}}</p>
    </van-sticky>
    <!--内容容器-->
    <van-pull-refresh v-model="isLoading" @refresh="onRefresh" loosing-text="释放加载上一章" pulling-text="下拉加载上一章">
      ...
      >
        <article class="content" @click="show = !show" :style="{'font-size':fontSize+'px'}">
          <section v-for="(item,index) in chapters" :key="index">
            <h3 class="col-12 cTitle">{{ item.title}}</h3>
            <div class="col-12" :class="{'white':isShwNight}" v-html="item.content"></div>
          </section>
        </article>
      </van-list>
    </van-pull-refresh>
